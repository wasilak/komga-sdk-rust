...
/// Required role: **ADMIN**
pub async fn get_announcements(configuration: &configuration::Configuration, ) -> Result<models::JsonFeedDto, Error<GetAnnouncementsError>> {

    let uri_str = format!("{}/api/v1/announcements", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::JsonFeedDto`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::JsonFeedDto`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn mark_announcements_read(configuration: &configuration::Configuration, request_body: Vec<String>) -> Result<(), Error<MarkAnnouncementsReadError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<MarkAnnouncementsReadError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
