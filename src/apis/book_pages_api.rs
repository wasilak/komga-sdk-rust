...
/// struct for typed errors of method [`get_book_page_by_number`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetBookPageByNumberError {
    Status400(models::ValidationErrorResponse),
    DefaultResponse(std::path::PathBuf),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`get_book_page_raw_by_number`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetBookPageRawByNumberError {
    Status400(models::ValidationErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`get_book_page_thumbnail_by_number`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetBookPageThumbnailByNumberError {
    Status400(models::ValidationErrorResponse),
    DefaultResponse(std::path::PathBuf),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_book_page_by_number(
    ...
    if let Some(ref param_value) = p_query_zero_based {
        req_builder = req_builder.query(&[("zero_based", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_query_content_negotiation {
        req_builder = req_builder.query(&[("contentNegotiation", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetBookPageByNumberError> = serde_json::from_str(&content).ok();
...
pub async fn get_book_page_raw_by_number(
    configuration: &configuration::Configuration,
    book_id: &str,
    page_number: i32,
) -> Result<String, Error<GetBookPageRawByNumberError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_book_id = book_id;
    let p_path_page_number = page_number;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetBookPageRawByNumberError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn get_book_page_thumbnail_by_number(
    configuration: &configuration::Configuration,
    book_id: &str,
    page_number: i32,
) -> Result<std::path::PathBuf, Error<GetBookPageThumbnailByNumberError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_book_id = book_id;
    let p_path_page_number = page_number;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetBookPageThumbnailByNumberError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn get_book_pages(
    configuration: &configuration::Configuration,
    book_id: &str,
) -> Result<Vec<models::PageDto>, Error<GetBookPagesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_book_id = book_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::PageDto&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::PageDto&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetBookPagesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
