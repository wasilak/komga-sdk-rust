...
/// struct for typed errors of method [`empty_task_queue`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum EmptyTaskQueueError {
...
pub async fn empty_task_queue(configuration: &configuration::Configuration, ) -> Result<i32, Error<EmptyTaskQueueError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<EmptyTaskQueueError> = serde_json::from_str(&content).ok();
