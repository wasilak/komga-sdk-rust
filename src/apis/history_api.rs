...
/// struct for typed errors of method [`get_historical_events`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetHistoricalEventsError {
...
/// Required role: **ADMIN**
pub async fn get_historical_events(configuration: &configuration::Configuration, page: Option<i32>, size: Option<i32>, sort: Option<Vec<String>>) -> Result<models::PageHistoricalEventDto, Error<GetHistoricalEventsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PageHistoricalEventDto`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PageHistoricalEventDto`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetHistoricalEventsError> = serde_json::from_str(&content).ok();
