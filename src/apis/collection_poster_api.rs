...
/// struct for typed errors of method [`mark_collection_thumbnail_selected`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum MarkCollectionThumbnailSelectedError {
    Status400(models::ValidationErrorResponse),
    UnknownValue(serde_json::Value),
}

/// Required role: **ADMIN**
pub async fn add_user_uploaded_collection_thumbnail(
    configuration: &configuration::Configuration,
    id: &str,
    file: std::path::PathBuf,
    selected: Option<bool>,
) -> Result<models::ThumbnailSeriesCollectionDto, Error<AddUserUploadedCollectionThumbnailError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ThumbnailSeriesCollectionDto`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ThumbnailSeriesCollectionDto`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<AddUserUploadedCollectionThumbnailError> =
...
/// Required role: **ADMIN**
pub async fn delete_user_uploaded_collection_thumbnail(
    configuration: &configuration::Configuration,
    id: &str,
    thumbnail_id: &str,
) -> Result<(), Error<DeleteUserUploadedCollectionThumbnailError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<DeleteUserUploadedCollectionThumbnailError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
...
pub async fn get_collection_thumbnail(
    configuration: &configuration::Configuration,
    id: &str,
) -> Result<std::path::PathBuf, Error<GetCollectionThumbnailError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetCollectionThumbnailError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn get_collection_thumbnail_by_id(
    configuration: &configuration::Configuration,
    id: &str,
    thumbnail_id: &str,
) -> Result<std::path::PathBuf, Error<GetCollectionThumbnailByIdError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    let p_path_thumbnail_id = thumbnail_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetCollectionThumbnailByIdError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn get_collection_thumbnails(
    configuration: &configuration::Configuration,
    id: &str,
) -> Result<Vec<models::ThumbnailSeriesCollectionDto>, Error<GetCollectionThumbnailsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::ThumbnailSeriesCollectionDto&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::ThumbnailSeriesCollectionDto&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetCollectionThumbnailsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
/// Required role: **ADMIN**
pub async fn mark_collection_thumbnail_selected(
    configuration: &configuration::Configuration,
    id: &str,
    thumbnail_id: &str,
) -> Result<(), Error<MarkCollectionThumbnailSelectedError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    let p_path_thumbnail_id = thumbnail_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<MarkCollectionThumbnailSelectedError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
