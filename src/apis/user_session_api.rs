...
/// struct for typed errors of method [`convert_header_session_to_cookie`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ConvertHeaderSessionToCookieError {
...
/// struct for typed errors of method [`post_logout1`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum PostLogout1Error {
    UnknownValue(serde_json::Value),
}


/// Forcefully return Set-Cookie header, even if the session is contained in the X-Auth-Token header.
pub async fn convert_header_session_to_cookie(configuration: &configuration::Configuration, ) -> Result<(), Error<ConvertHeaderSessionToCookieError>> {

    let uri_str = format!("{}/api/v1/login/set-cookie", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<ConvertHeaderSessionToCookieError> = serde_json::from_str(&content).ok();
...
/// Invalidates the current session and clean up any remember-me authentication.
pub async fn post_logout1(configuration: &configuration::Configuration, ) -> Result<(), Error<PostLogout1Error>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<PostLogout1Error> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
