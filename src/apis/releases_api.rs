...
/// Required role: **ADMIN**
pub async fn get_releases(configuration: &configuration::Configuration, ) -> Result<Vec<models::ReleaseDto>, Error<GetReleasesError>> {

    let uri_str = format!("{}/api/v1/releases", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::ReleaseDto&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::ReleaseDto&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
