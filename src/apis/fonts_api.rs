...
/// struct for typed errors of method [`get_font_family_as_css`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetFontFamilyAsCssError {
    Status400(models::ValidationErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`get_font_file`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetFontFileError {
    Status400(models::ValidationErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`get_fonts`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn get_font_family_as_css(configuration: &configuration::Configuration, font_family: &str) -> Result<reqwest::Response, Error<GetFontFamilyAsCssError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_font_family = font_family;

    let uri_str = format!("{}/api/v1/fonts/resource/{fontFamily}/css", configuration.base_path, fontFamily=crate::apis::urlencode(p_path_font_family));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<GetFontFamilyAsCssError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

pub async fn get_font_file(configuration: &configuration::Configuration, font_family: &str, font_file: &str) -> Result<reqwest::Response, Error<GetFontFileError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_font_family = font_family;
    let p_path_font_file = font_file;

    let uri_str = format!("{}/api/v1/fonts/resource/{fontFamily}/{fontFile}", configuration.base_path, fontFamily=crate::apis::urlencode(p_path_font_family), fontFile=crate::apis::urlencode(p_path_font_file));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetFontFileError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// List all available font families.
pub async fn get_fonts(configuration: &configuration::Configuration, ) -> Result<Vec<String>, Error<GetFontsError>> {

    let uri_str = format!("{}/api/v1/fonts/families", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetFontsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
