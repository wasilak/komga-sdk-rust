...
/// struct for typed errors of method [`get_directory_listing`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetDirectoryListingError {
...
pub async fn get_directory_listing(configuration: &configuration::Configuration, directory_request_dto: Option<models::DirectoryRequestDto>) -> Result<models::DirectoryListingDto, Error<GetDirectoryListingError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_directory_request_dto = directory_request_dto;
    ...
    req_builder = req_builder.json(&p_body_directory_request_dto);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::DirectoryListingDto`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::DirectoryListingDto`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetDirectoryListingError> = serde_json::from_str(&content).ok();
