...
/// struct for typed errors of method [`get_actuator_info`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetActuatorInfoError {
    UnknownValue(serde_json::Value),
}


/// Required role: **ADMIN**
pub async fn get_actuator_info(configuration: &configuration::Configuration, ) -> Result<(), Error<GetActuatorInfoError>> {

    let uri_str = format!("{}/actuator/info", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<GetActuatorInfoError> = serde_json::from_str(&content).ok();
