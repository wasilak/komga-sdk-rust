...
/// struct for typed errors of method [`match_comic_rack_list`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum MatchComicRackListError {
...
/// Required role: **ADMIN**
pub async fn match_comic_rack_list(configuration: &configuration::Configuration, add_user_uploaded_book_thumbnail_request: Option<models::AddUserUploadedBookThumbnailRequest>) -> Result<models::ReadListRequestMatchDto, Error<MatchComicRackListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_add_user_uploaded_book_thumbnail_request = add_user_uploaded_book_thumbnail_request;

    let uri_str = format!("{}/api/v1/readlists/match/comicrack", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_add_user_uploaded_book_thumbnail_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ReadListRequestMatchDto`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ReadListRequestMatchDto`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<MatchComicRackListError> = serde_json::from_str(&content).ok();
