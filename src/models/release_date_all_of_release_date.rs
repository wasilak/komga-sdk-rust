...
#[serde(untagged)]
pub enum ReleaseDateAllOfReleaseDate {
    After(Box<models::After>),
    Before(Box<models::Before>),
    IsInTheLast(Box<models::IsInTheLast>),
    IsNotInTheLast(Box<models::IsNotInTheLast>),
    IsNotNull(Box<models::IsNotNull>),
    IsNull(Box<models::IsNull>),
}

impl Default for ReleaseDateAllOfReleaseDate {
    fn default() -> Self {
        Self::After(Default::default())
    }
}
