...
#[serde(untagged)]
pub enum AllOfBookAllOfAllOf {
    AllOfBook(Box<models::AllOfBook>),
    AnyOfBook(Box<models::AnyOfBook>),
    Author(Box<models::Author>),
    Deleted(Box<models::Deleted>),
    LibraryId(Box<models::LibraryId>),
    MediaProfile(Box<models::MediaProfile>),
    MediaStatus(Box<models::MediaStatus>),
    NumberSort(Box<models::NumberSort>),
    OneShot(Box<models::OneShot>),
    Poster(Box<models::Poster>),
    ReadListId(Box<models::ReadListId>),
    ReadStatus(Box<models::ReadStatus>),
    ReleaseDate(Box<models::ReleaseDate>),
    SeriesId(Box<models::SeriesId>),
    Tag(Box<models::Tag>),
    Title(Box<models::Title>),
}

impl Default for AllOfBookAllOfAllOf {
    fn default() -> Self {
        Self::AllOfBook(Default::default())
    }
}
